...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class RiskCalculator : IDecisionComponent
    {
        ...
        private void CalculateStructuralRiskLevels(HeatZone zone, IBarDataProvider barData, CoreEngine coreEngine, int currentBar, double accountSize)
        {
            ...
            // FIX: currentBar viene del TF más bajo (5m), necesitamos el índice correcto para zone.TFDominante
            int currentBarZoneTF = barData.GetCurrentBarIndex(zone.TFDominante);
            double atr = barData.GetATR(zone.TFDominante, 14, currentBarZoneTF);
            if (atr <= 0)
            {
            ...
            {
                // FIX: Ya tenemos currentBarZoneTF declarado arriba, solo lo usamos
                double currentPrice = barData.GetClose(zone.TFDominante, currentBarZoneTF);
            ...
            if (slDistanceATR > _config.MaxSLDistanceATR)
            {
                ...
                {
                    // FIX: Ya tenemos currentBarZoneTF declarado arriba, solo lo usamos
                    double currentPriceDbg = barData.GetClose(zone.TFDominante, currentBarZoneTF);
                    _logger.Info(string.Format(
        ...
        {
            // FIX: currentBar viene del TF más bajo (5m), necesitamos el índice correcto para timeframeMinutes
            int currentBarTF = barData.GetCurrentBarIndex(timeframeMinutes);
            double atr = barData.GetATR(timeframeMinutes, 14, currentBarTF);
            if (atr <= 0)
            {
        ...
        private double CalculateStructuralTP_Buy(HeatZone zone, CoreEngine coreEngine, IBarDataProvider barData, int currentBar, double entry, double stopLoss)
        {
            ...
            // FIX: Declarar y obtener índice correcto para zone.TFDominante
            int currentBarZoneTF = barData.GetCurrentBarIndex(zone.TFDominante);
            double atr = barData.GetATR(zone.TFDominante, 14, currentBarZoneTF);
