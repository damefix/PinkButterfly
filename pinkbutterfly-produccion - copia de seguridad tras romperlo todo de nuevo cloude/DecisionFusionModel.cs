...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class DecisionFusionModel : IDecisionComponent
    {
        ...
        private int _proxDiagCounter = 0;
        ...
        public void Process(DecisionSnapshot snapshot, IBarDataProvider barData, CoreEngine coreEngine, int currentBar, int timeframeMinutes, double accountSize)
        {
            ...
            foreach (var zone in validZones)
            {
                ...
                _proxDiagCounter++;
                const int sampleRate = 50; // 1 de cada 50 evaluaciones
                if ((_proxDiagCounter % sampleRate) == 0)
                {
                    try
                    {
                        ...
                        double distATR = atr > 0 ? distPts / atr : 0.0;
                        double proxFactor = zone.Metadata.ContainsKey("ProximityFactor") ? (double)zone.Metadata["ProximityFactor"] : -1.0;
                        bool aligned = zone.Direction == snapshot.GlobalBias;
                        _logger.Info(string.Format(
                            "[DIAG][DFM][PROX] Zone={0} TF={1} Dir={2} DistATR={3:F2} Prox={4:F3} Core={5:F3} Conf={6:F3} Aligned={7}",
                            zone.Id, zone.TFDominante, zone.Direction, distATR, proxFactor, zone.Score, breakdown.FinalConfidence, aligned));
                    }
                    catch { }
                }

                // Trazas compactas [DFM]: TF, distancia, ATR, proximidad y confidence
                if (_config.EnableDebug)
                {
                    try
                    {
                        ...
                        double distPts = cur > 0 ? Math.Abs(zone.CenterPrice - cur) : 0.0;
                        double distATR = atr > 0 ? distPts / atr : 0.0;
                        double proxScore = zone.Metadata.ContainsKey("ProximityScore") ? (double)zone.Metadata["ProximityScore"] : -1.0;
                        double proxFactor = zone.Metadata.ContainsKey("ProximityFactor") ? (double)zone.Metadata["ProximityFactor"] : -1.0;
                        double zoneInfluence = breakdown.CoreScoreContribution + breakdown.ProximityContribution + breakdown.ConfluenceContribution + breakdown.TypeContribution + breakdown.BiasContribution + breakdown.MomentumContribution + breakdown.VolumeContribution;
                        _logger.Debug(string.Format(
                            "[DFM] Zone {0} TF={1}m Dist={2:F2} pts ({3:F2} ATR) ProxScore={4:F3} ProxFactor={5:F3} Influence={6:F3} FinalConf={7:F3}",
                            zone.Id, zone.TFDominante, distPts, distATR, proxScore, proxFactor, zoneInfluence, breakdown.FinalConfidence));
                    }
                    catch { }
                }
            ...
            _logger.Info(string.Format("[DIAG][DFM] Evaluadas: Bull={0} Bear={1} | PassedThreshold={2}", evalBull, evalBear, passed));
            _logger.Info(string.Format("[DIAG][DFM] ConfidenceBins: {0}", bins));
        }
