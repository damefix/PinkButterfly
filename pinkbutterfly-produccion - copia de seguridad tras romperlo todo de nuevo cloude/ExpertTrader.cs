...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class ExpertTrader : Indicator
    {
        ...
        // Ancla temporal para barsToSkip independiente del TF del gráfico (preroll + anclaje por tiempo)
        private bool _startAnchored = false;
        private DateTime _startTimeDecision = DateTime.MinValue;
        private Dictionary<int, int> _barsToSkipPerTF = new Dictionary<int, int>();
        private int _decisionTFIndex = -1;
        private int _startAbsIndexDecision = -1; // Índice absoluto donde empieza la ventana de análisis
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                try
                {
                    ...
                    // Encontrar el índice del TF de decisión en BarsArray
                    for (int i = 0; i < BarsArray.Length; i++)
                    {
                        if (BarsArray[i] != null && BarsArray[i].BarsPeriod.BarsPeriodType == BarsPeriodType.Minute 
                            && BarsArray[i].BarsPeriod.Value == EngineConfig.DECISION_TIMEFRAME_MINUTES)
                        {
                            _decisionTFIndex = i;
                            break;
                        }
                    ...
                    {
                        Print($"[ExpertTrader] ⚠️ ERROR: TF de decisión {EngineConfig.DECISION_TIMEFRAME_MINUTES}m no encontrado en BarsArray");
                    }
                    else
                    {
                        // Calcular el índice absoluto de inicio (no relativo a CurrentBars)
                        int totalBarsDecision = BarsArray[_decisionTFIndex].Count;
                        _startAbsIndexDecision = totalBarsDecision - _config.BacktestBarsForAnalysis;
                        
                        Print($"[ExpertTrader] TF de decisión {EngineConfig.DECISION_TIMEFRAME_MINUTES}m encontrado en BarsArray[{_decisionTFIndex}]");
                        Print($"[ExpertTrader] Total barras en {EngineConfig.DECISION_TIMEFRAME_MINUTES}m: {totalBarsDecision}");
                        Print($"[ExpertTrader] Índice absoluto de inicio: {_startAbsIndexDecision}");
                        Print($"[ExpertTrader] Preroll: esperando CurrentBars[{_decisionTFIndex}] == {_startAbsIndexDecision}");
                    }
        ...
        protected override void OnBarUpdate()
        {
            try
            {
                ...
                // TRAZA: Entrada a OnBarUpdate
                if (_fileLogger != null)
                    _fileLogger.Info($"[GATE] enter bip={barsInProgressIndex}, tf={tfMinutes}m, CB={CurrentBars[barsInProgressIndex]}, decisionCB={CurrentBars[_decisionTFIndex]}, anchored={_startAnchored}");
                
            // 5. PREROLL + ANCLAJE POR TIEMPO (solución definitiva NinjaTrader MTF)
            // Anclamos cuando CurrentBars[decisionIndex] == _startAbsIndexDecision
            
            // 5.1. Verificar que el TF de decisión esté disponible
            if (_decisionTFIndex < 0 || _startAbsIndexDecision < 0)
            {
                Print($"[ExpertTrader] ERROR: TF de decisión no configurado correctamente");
                return;
            }
            
            // 5.2. FASE PREROLL: NO procesar NINGÚN TF hasta que el TF de decisión llegue al índice correcto
            if (!_startAnchored)
            {
                // GATE GLOBAL DE PREROLL: Si el TF de decisión aún no ha llegado, skip TODO
                if (CurrentBars[_decisionTFIndex] < _startAbsIndexDecision)
                {
                    // TRAZA: Gate preroll activo
                    if (_fileLogger != null)
                        _fileLogger.Info($"[GATE] preroll return - tf={tfMinutes}m, decisionCB={CurrentBars[_decisionTFIndex]} < startAbs={_startAbsIndexDecision}");
                    return; // Preroll global: NO procesar ningún TF (ni 5m, ni 15m, ni 60m, etc.)
                }
                
                // Solo anclar cuando llegue un tick del TF de decisión en el índice exacto
                if (barsInProgressIndex == _decisionTFIndex && CurrentBars[_decisionTFIndex] == _startAbsIndexDecision)
                {
                    // ANCLAR EL TIEMPO: usar Times[decisionIndex][0] (la barra actual)
                    _startTimeDecision = Times[_decisionTFIndex][0];
                    _startAnchored = true;
                    
                    // CRÍTICO: Fijar el skip del TF de decisión para no perder esta barra
                    _barsToSkipPerTF[EngineConfig.DECISION_TIMEFRAME_MINUTES] = _startAbsIndexDecision;
                    
                    if (_fileLogger != null)
                        _fileLogger.Info($"[ANCHOR] ✅ ANCLA TEMPORAL FIJADA: {_startTimeDecision:yyyy-MM-dd HH:mm:ss}, TF={EngineConfig.DECISION_TIMEFRAME_MINUTES}m, StartAbsIdx={_startAbsIndexDecision}, CurrentBars={CurrentBars[_decisionTFIndex]}, skip[15]={_startAbsIndexDecision}");
                    
                    Print($"[ExpertTrader] ═══════════════════════════════════════════════════════");
                    Print($"[ExpertTrader] ✅ ANCLA TEMPORAL FIJADA");
                    Print($"[ExpertTrader] ═══════════════════════════════════════════════════════");
                    Print($"[ExpertTrader] Tiempo de inicio: {_startTimeDecision:yyyy-MM-dd HH:mm:ss}");
                    Print($"[ExpertTrader] TF decisión: {EngineConfig.DECISION_TIMEFRAME_MINUTES}m");
                    Print($"[ExpertTrader] StartAbsIdx = {_startAbsIndexDecision}, CurrentBars[{_decisionTFIndex}] = {CurrentBars[_decisionTFIndex]}");
                    Print($"[ExpertTrader] Barras a procesar: {_config.BacktestBarsForAnalysis}");
                    Print($"[ExpertTrader] ═══════════════════════════════════════════════════════");
                    
                    // NO hacer return aquí: procesar esta barra de 15m inmediatamente
                }
                else
                {
                    // Durante preroll, otros TFs también deben hacer return
                    return; // Preroll: esperar a que el TF de decisión ancle
                }
            }
            
            // 5.3. GATE PRE-CÁLCULO: No procesar nada si este TF aún no ha llegado al ancla temporal
            // Este gate se ejecuta ANTES del cálculo lazy para prevenir procesamiento prematuro
            // OPTIMIZACIÓN: El TF de decisión (15m) ya tiene su skip fijado en el bloque de anclaje
            if (_startAnchored && !_barsToSkipPerTF.ContainsKey(tfMinutes))
            {
                // Si este TF aún no ha llegado al tiempo de inicio, NO procesar
                DateTime currentBarTime = Times[barsInProgressIndex][0];
                if (currentBarTime < _startTimeDecision)
                {
                    // TRAZA: Gate pre-cálculo activo
                    if (_fileLogger != null)
                        _fileLogger.Info($"[GATE] pre-calc return - tf={tfMinutes}m esperando datos >= startTime");
                    return; // Skip silencioso: esperando que el TF cruce el ancla
                }
                
                // AHORA SÍ calcular skip (el TF ya tiene datos >= startTimeDecision)
                if (_fileLogger != null)
                    _fileLogger.Info($"[SKIP] TF {tfMinutes}m: Primera barra >= {_startTimeDecision:yyyy-MM-dd HH:mm:ss}, calculando skip...");
                Print($"[ExpertTrader] TF {tfMinutes}m: Primera barra >= {_startTimeDecision:yyyy-MM-dd HH:mm:ss}, calculando skip...");
                // Encontrar el índice del TF en BarsArray
                int tfIndex = -1;
                for (int i = 0; i < BarsArray.Length; i++)
                {
                    if (BarsArray[i] != null && BarsArray[i].BarsPeriod.BarsPeriodType == BarsPeriodType.Minute 
                        && BarsArray[i].BarsPeriod.Value == tfMinutes)
                    {
                        tfIndex = i;
                        break;
                    }
                ...
                {
                    if (_fileLogger != null)
                        _fileLogger.Error($"[SKIP] ERROR: TF {tfMinutes}m no encontrado en BarsArray");
                    Print($"[ExpertTrader] ERROR: TF {tfMinutes}m no encontrado en BarsArray");
                    _barsToSkipPerTF[tfMinutes] = 0;
                    return; // ← FIX CRÍTICO: Return para no entrar al else
                }
                else
                {
                    // Buscar la primera barra (más antigua) con Time >= _startTimeDecision
                    // Times[i][barsAgo]: barsAgo=alto=antigua, barsAgo=0=reciente
                    int currentBarsTF = CurrentBars[tfIndex];
                    if (_fileLogger != null)
                        _fileLogger.Info($"[SKIP] TF {tfMinutes}m: tfIndex={tfIndex}, CurrentBars={currentBarsTF}");
                    int skipForThisTF = 0;
                    DateTime firstValidTime = DateTime.MinValue;
                    bool found = false;
                    
                    // Buscar desde la barra más antigua hacia la más reciente
                    for (int barsAgo = currentBarsTF; barsAgo >= 0; barsAgo--)
                    {
                        DateTime barTime = Times[tfIndex][barsAgo];
                        
                        // Encontrar la primera (más antigua) con tiempo >= startTimeDecision
                        if (barTime >= _startTimeDecision)
                        {
                            // El índice absoluto es currentBarsTF - barsAgo
                            skipForThisTF = currentBarsTF - barsAgo;
                            firstValidTime = barTime;
                            found = true;
                            break;
                        }
                    }
                    ...
                    {
                        if (_fileLogger != null)
                            _fileLogger.Warning($"[SKIP] TF {tfMinutes}m: Todas las barras son < {_startTimeDecision:yyyy-MM-dd HH:mm:ss}, skip=0");
                        Print($"[ExpertTrader] WARNING: TF {tfMinutes}m: Todas las barras son < {_startTimeDecision:yyyy-MM-dd HH:mm:ss}, skip=0");
                        skipForThisTF = 0;
                    }
                    else
                    {
                        if (_fileLogger != null)
                            _fileLogger.Info($"[SKIP] ✅ TF {tfMinutes}m: skip={skipForThisTF}, firstTime={firstValidTime:yyyy-MM-dd HH:mm:ss} >= start={_startTimeDecision:yyyy-MM-dd HH:mm:ss}");
                        Print($"[ExpertTrader] ✅ TF {tfMinutes}m: skip={skipForThisTF}, firstTime={firstValidTime:yyyy-MM-dd HH:mm:ss} >= start={_startTimeDecision:yyyy-MM-dd HH:mm:ss}");
                    }
                    
                    _barsToSkipPerTF[tfMinutes] = skipForThisTF;
                    
                    if (_fileLogger != null)
                        _fileLogger.Info($"[SKIP] TF {tfMinutes}m: guardado en _barsToSkipPerTF[{tfMinutes}]={skipForThisTF}");
                    
                    // ← FIX CRÍTICO 3: Return después de calcular skip para empezar en la SIGUIENTE barra
                    return;
                }
            }
            
            // 5.4. APLICAR SKIP: No procesar barras anteriores al índice de inicio
            if (_startAnchored && _barsToSkipPerTF.ContainsKey(tfMinutes))
            {
                if (CurrentBars[barsInProgressIndex] < _barsToSkipPerTF[tfMinutes])
                {
                    // TRAZA: Gate definitivo activo
                    if (_fileLogger != null)
                        _fileLogger.Info($"[GATE] skip return - tf={tfMinutes}m, CB={CurrentBars[barsInProgressIndex]} < skip={_barsToSkipPerTF[tfMinutes]}");
                    return; // Skip: aún no hemos llegado al tiempo de inicio en este TF
                }
            }
                
            // 6. Control de logging: basado en el TF de decisión
            bool enableLogging = (State == State.Realtime);
            if (State == State.Historical && _startAnchored && _startTimeDecision != DateTime.MinValue)
            {
                DateTime currentBarTime = Times[barsInProgressIndex][0];
                int currentIdxDecisionTF = _barDataProvider.GetBarIndexFromTime(EngineConfig.DECISION_TIMEFRAME_MINUTES, currentBarTime);
                int totalBarsDecisionTF = _barDataProvider.GetCurrentBarIndex(EngineConfig.DECISION_TIMEFRAME_MINUTES);
                
                enableLogging = (currentIdxDecisionTF >= totalBarsDecisionTF - _config.LoggingThresholdBars);
            }
            ...
            if (_config.TimeframesToUse.Contains(tfMinutes))
                {
                    // TRAZA: Llamada al CoreEngine
                    if (_fileLogger != null)
                        _fileLogger.Info($"[CALL] CoreEngine.OnBarClose tf={tfMinutes}m, bar={barIndex}");
                ...
                // 8. Generar decisión solo en el TF configurado (15m), independiente del TF del gráfico
                // Esto permite que el sistema funcione igual con cualquier TF de gráfico (5m, 15m, 60m, etc.)
                if (tfMinutes == EngineConfig.DECISION_TIMEFRAME_MINUTES && barIndex >= 20)
                {
