...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class ContextManager : IDecisionComponent
    {
        ...
        private void BuildDecisionSummary(DecisionSnapshot snapshot, IBarDataProvider barData, CoreEngine coreEngine, int currentBar)
        {
            ...
            // CurrentPrice: Precio de cierre de la barra actual en el TF de decisión
            // FIX: currentBar viene del TF más bajo (5m), necesitamos el índice correcto para el TF de decisión
            int decisionTF = EngineConfig.DECISION_TIMEFRAME_MINUTES;
            ...
            {
                // FIX: currentBar viene del TF más bajo (5m), necesitamos el índice correcto para cada TF
        ...
        private void CalculateGlobalBias(DecisionSnapshot snapshot, CoreEngine coreEngine, int currentBar)
        {
            ...
            // FIX: currentBar viene del TF de decisión, necesitamos el índice correcto para primaryTF
            int currentBarPrimaryTF = _barData.GetCurrentBarIndex(primaryTF);
            
            // CRÍTICO: Obtener CurrentPrice del MISMO TF que usamos para el promedio
            double currentPrice = _barData.GetClose(primaryTF, currentBarPrimaryTF);
            double sumPrices = 0.0;
            int period = 200;
            int validBars = 0;

            for (int i = 0; i < period && (currentBarPrimaryTF - i) >= 0; i++)
            {
                sumPrices += _barData.GetClose(primaryTF, currentBarPrimaryTF - i);
                validBars++;
            }
        ...
        private void CalculateMarketVolatility(DecisionSnapshot snapshot, IBarDataProvider barData, int currentBar)
        {
            ...
            // FIX: currentBar viene del TF más bajo (5m), necesitamos el índice correcto para primaryTF
            int currentBarPrimaryTF = barData.GetCurrentBarIndex(primaryTF);

            // ATR actual (14 períodos)
            double atrActual = barData.GetATR(primaryTF, 14, currentBarPrimaryTF);

            // ATR de largo plazo (200 períodos)
            double atrLongTerm = barData.GetATR(primaryTF, 200, currentBarPrimaryTF);
