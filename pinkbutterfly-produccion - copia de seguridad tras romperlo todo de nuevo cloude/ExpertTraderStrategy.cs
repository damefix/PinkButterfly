    ...
    public class ExpertTraderStrategy : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                ...
                Print("");
                Print("Configurando indicador ExpertTrader...");
                
                // Crear instancia del indicador manualmente
                _indicator = new Indicators.PinkButterfly.ExpertTrader();
                _indicator.Language = this.Language;
                _indicator.AccountSize = this.AccountSize;
                _indicator.ShowEntryLines = false;
                _indicator.ShowSLTPLines = false;
                _indicator.ShowPanel = false;
                _indicator.EntryLineWidth = 2;
                _indicator.SLTPLineWidth = 2;
                _indicator.EnableFastLoad = this.EnableFastLoad;
                _indicator.ContractSize = this.ContractSize;
                _indicator.EnableOutputLogging = this.EnableOutputLogging;
                _indicator.EnableFileLogging = this.EnableFileLogging;
                _indicator.EnableTradeCSV = this.EnableTradeCSV;
                
                // Añadir el indicador a la estrategia
                AddChartIndicator(_indicator);
                
                Print("Indicador ExpertTrader configurado correctamente.");
