...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class LiquidityVoidDetector : IDetector
    {
        ...
        private void DetectVoids(int tfMinutes, int barIndex)
        {
            ...
            _logger.Debug($"LV: TF{tfMinutes} bar{barIndex} - A[{idxA}]: H={highA:F2} L={lowA:F2}, B[{idxB}]: H={highB:F2} L={lowB:F2}");

            // Calcular ATR para validación
            double atr = _provider.GetATR(tfMinutes, 14, barIndex);
            if (atr <= 0)
            {
                _logger.Debug($"LV: ATR inválido={atr} en TF{tfMinutes} bar{barIndex}");
                return;
            }

            double tickSize = _provider.GetTickSize();
            double minVoidSize = _config.LV_MinSizeATRFactor * atr;
            _logger.Debug($"LV: ATR={atr:F2}, minVoidSize={minVoidSize:F2} (factor={_config.LV_MinSizeATRFactor})");

            // Detectar Bullish Void: highA < lowB (gap hacia arriba)
            if (highA < lowB)
            {
                double voidSize = lowB - highA;
                _logger.Debug($"LV: Bullish gap detected! voidSize={voidSize:F2}, minRequired={minVoidSize:F2}");
                ...
                if (voidSize >= minVoidSize)
                {
                    ...
                    _logger.Debug($"LV: Volume check - RequireLowVolume={_config.LV_RequireLowVolume}, volumeOk={volumeOk}");
                    ...
                    if (volumeOk)
                    {
                        ...
                        {
                            _logger.Debug($"LV: ✗ Bullish void rejected - FVG structure prevails (hierarchical exclusion)");
                        }
                    }
                    else
                    {
                        _logger.Debug($"LV: ✗ Bullish void rejected - volume too high");
                    }
                }
                else
                {
                    _logger.Debug($"LV: ✗ Bullish gap too small ({voidSize:F2} < {minVoidSize:F2})");
                }
            }
            ...
            else if (lowA > highB)
            {
                ...
                _logger.Debug($"LV: Bearish gap detected! voidSize={voidSize:F2}, minRequired={minVoidSize:F2}");
                ...
                if (voidSize >= minVoidSize)
                {
                    ...
                    if (volumeOk)
                    {
                        ...
                        {
                            _logger.Debug($"LV: ✗ Bearish void rejected - FVG structure prevails (hierarchical exclusion)");
                        }
                    }
                    else
                    {
                        _logger.Debug($"LV: ✗ Bearish void rejected - volume too high");
                    }
                }
                else
                {
                    _logger.Debug($"LV: ✗ Bearish gap too small ({voidSize:F2} < {minVoidSize:F2})");
                }
            }
        }
        ...
        private bool ExistsFVGInZone(int tfMinutes, double zoneLow, double zoneHigh)
        {
            ...
            foreach (var fvg in fvgs)
            {
                ...
                if (fvgContainsVoid)
                {
                    _logger.Debug($"LV: FVG [{fvg.Low:F2}, {fvg.High:F2}] contiene completamente void [{zoneLow:F2}, {zoneHigh:F2}]");
                    return true;
                }
            }
                                ...
                                double low, double high, double voidSize, double atr, double tickSize)
        {
            ...
            if (_config.LV_EnableFusion)
            {
                ...
                if (mergedVoid != null)
                {
                    _logger.Debug($"LiquidityVoidDetector: Void fusionado en TF{tfMinutes} - {mergedVoid.Id}");
            ...
            _logger.Debug($"LiquidityVoidDetector: Nuevo {direction} Void en TF{tfMinutes} [{low:F2}-{high:F2}] Score:{voidInfo.Score:F3}");
        }
        ...
        {
            double midPrice = _provider.GetMidPrice();
                ...
                if (bodyTouch || wickTouch)
                {
                    ...
                    if (totalRange > 0)
                    {
                        ...
                        if (voidInfo.FillPercentage >= _config.LV_FillThreshold)
                        {
                            ...
                            _logger.Debug($"LiquidityVoidDetector: Void {voidInfo.Id} rellenado ({voidInfo.FillPercentage:P0})");
                        }
                    }
                }
