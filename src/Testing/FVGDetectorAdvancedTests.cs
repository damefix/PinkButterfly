    ...
    public class FVGDetectorAdvancedTests
    {
        ...
        private void Test_Scoring_ProximityExtreme()
        {
            ...
            if (fvgs.Count > 0)
            {
                ...
                Assert(fvgs[0].Score < 0.1, 
                       "Scoring_ProximityExtreme_LowScore", 
                       $"Expected low score (<0.1) for distant FVG, got {fvgs[0].Score:F3}");

                _logger($"  Distant FVG score: {fvgs[0].Score:F3}");
            }
        }
        ...
        private void Test_Scoring_MultipleTimeframes()
        {
            ...
            if (fvgs60.Count > 0 && fvgs240.Count > 0)
            {
                ...
                Assert(fvgs240[0].Score > fvgs60[0].Score, 
                       "Scoring_MultipleTimeframes_HigherTFHigherScore", 
                       $"Expected TF240 score ({fvgs240[0].Score:F3}) > TF60 score ({fvgs60[0].Score:F3})");

                _logger($"  TF60 score: {fvgs60[0].Score:F3}, TF240 score: {fvgs240[0].Score:F3}");
            }
        }
