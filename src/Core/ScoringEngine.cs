    ...
    public class ScoringEngine
    {
        ...
        public double CalculateScore(StructureBase structure, int currentBarIndex, string marketBias, int confluenceCount = 1)
        {
            try
            {
                ...
                double freshness = Math.Exp(-ageBars / (double)_config.FreshnessLambda);

                // ============================================================
                // 3. PROXIMITY (distancia al precio actual)
                // ============================================================
                double currentPrice = _provider.GetMidPrice();
                double centerPrice = structure.CenterPrice;
                double distanceTicks = Math.Abs(currentPrice - centerPrice) / _provider.GetTickSize();
                ...
                double proximity = 1.0 - Math.Min(distanceTicks / proxMaxTicks, 1.0);
