...
def parse_log(log_path: str) -> dict:
    ...
    # Proximity drivers summary (INFO) - FIX: usar ~ en vez de ≈
    ...
    if args.output:
        try:
            with open(args.output, 'w', encoding='utf-8') as f:
                f.write(md)
            # FIX: Evitar emojis en stdout para Windows cp1252
            print(f"[OK] Informe generado: {args.output}")
        except Exception as e:
            # FIX: Guardar en archivo incluso si hay error
            try:
                with open(args.output, 'w', encoding='utf-8') as f:
                    f.write(md)
                print(f"[OK] Informe guardado (con warnings): {args.output}")
            except:
                pass
            print(f"ERROR guardando informe: {e}", file=sys.stderr)
    else:
        # FIX: Para output a stdout, escribir en modo binario UTF-8
