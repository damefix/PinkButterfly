...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private PinkButterfly.ExpertTrader[] cacheExpertTrader;
		public PinkButterfly.ExpertTrader ExpertTrader(LanguageOption language, double accountSize, bool showEntryLines, bool showSLTPLines, bool showPanel, int entryLineWidth, int sLTPLineWidth, bool enableFastLoad, int contractSize, bool enableOutputLogging, bool enableFileLogging, bool enableTradeCSV)
		{
			return ExpertTrader(Input, language, accountSize, showEntryLines, showSLTPLines, showPanel, entryLineWidth, sLTPLineWidth, enableFastLoad, contractSize, enableOutputLogging, enableFileLogging, enableTradeCSV);
		}

		public PinkButterfly.ExpertTrader ExpertTrader(ISeries<double> input, LanguageOption language, double accountSize, bool showEntryLines, bool showSLTPLines, bool showPanel, int entryLineWidth, int sLTPLineWidth, bool enableFastLoad, int contractSize, bool enableOutputLogging, bool enableFileLogging, bool enableTradeCSV)
		{
			if (cacheExpertTrader != null)
				for (int idx = 0; idx < cacheExpertTrader.Length; idx++)
					if (cacheExpertTrader[idx] != null && cacheExpertTrader[idx].Language == language && cacheExpertTrader[idx].AccountSize == accountSize && cacheExpertTrader[idx].ShowEntryLines == showEntryLines && cacheExpertTrader[idx].ShowSLTPLines == showSLTPLines && cacheExpertTrader[idx].ShowPanel == showPanel && cacheExpertTrader[idx].EntryLineWidth == entryLineWidth && cacheExpertTrader[idx].SLTPLineWidth == sLTPLineWidth && cacheExpertTrader[idx].EnableFastLoad == enableFastLoad && cacheExpertTrader[idx].ContractSize == contractSize && cacheExpertTrader[idx].EnableOutputLogging == enableOutputLogging && cacheExpertTrader[idx].EnableFileLogging == enableFileLogging && cacheExpertTrader[idx].EnableTradeCSV == enableTradeCSV && cacheExpertTrader[idx].EqualsInput(input))
						return cacheExpertTrader[idx];
			return CacheIndicator<PinkButterfly.ExpertTrader>(new PinkButterfly.ExpertTrader(){ Language = language, AccountSize = accountSize, ShowEntryLines = showEntryLines, ShowSLTPLines = showSLTPLines, ShowPanel = showPanel, EntryLineWidth = entryLineWidth, SLTPLineWidth = sLTPLineWidth, EnableFastLoad = enableFastLoad, ContractSize = contractSize, EnableOutputLogging = enableOutputLogging, EnableFileLogging = enableFileLogging, EnableTradeCSV = enableTradeCSV }, input, ref cacheExpertTrader);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.PinkButterfly.ExpertTrader ExpertTrader(LanguageOption language, double accountSize, bool showEntryLines, bool showSLTPLines, bool showPanel, int entryLineWidth, int sLTPLineWidth, bool enableFastLoad, int contractSize, bool enableOutputLogging, bool enableFileLogging, bool enableTradeCSV)
		{
			return indicator.ExpertTrader(Input, language, accountSize, showEntryLines, showSLTPLines, showPanel, entryLineWidth, sLTPLineWidth, enableFastLoad, contractSize, enableOutputLogging, enableFileLogging, enableTradeCSV);
		}

		public Indicators.PinkButterfly.ExpertTrader ExpertTrader(ISeries<double> input , LanguageOption language, double accountSize, bool showEntryLines, bool showSLTPLines, bool showPanel, int entryLineWidth, int sLTPLineWidth, bool enableFastLoad, int contractSize, bool enableOutputLogging, bool enableFileLogging, bool enableTradeCSV)
		{
			return indicator.ExpertTrader(input, language, accountSize, showEntryLines, showSLTPLines, showPanel, entryLineWidth, sLTPLineWidth, enableFastLoad, contractSize, enableOutputLogging, enableFileLogging, enableTradeCSV);
		}
	}
