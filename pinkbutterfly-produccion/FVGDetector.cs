...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class FVGDetector : IDetector
    {
                               ...
                               double low, double high, int idxA, int idxB, int idxC)
        {
            ...
            // Calcular y fijar score inicial con bonus TF aplicado (controlado por flag)
            if (_config.EnableFVGInitialScoreOnCreation)
            {
                try
                {
        ...
        private void UpdateExistingFVGs(int tfMinutes, int barIndex)
        {
            ...
            var activeFvgs = _fvgCacheByTF[tfMinutes]
                .Where(f => f.IsActive)
                .ToList();
            var fvgsToRemove = new List<FVGInfo>();
            ...
            foreach (var fvg in activeFvgs)
            {
                ...
                {
                    // La estructura fue purgada, marcar para remover del caché local
                    fvgsToRemove.Add(fvg);
                }
                }
            // Remover purgadas del caché local
            if (fvgsToRemove.Count > 0)
            {
                foreach (var rem in fvgsToRemove)
                {
                    _fvgCacheByTF[tfMinutes].Remove(rem);
                }
            }
            }
        }
