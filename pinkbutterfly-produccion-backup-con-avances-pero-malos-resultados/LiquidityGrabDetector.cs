...
namespace NinjaTrader.NinjaScript.Indicators.PinkButterfly
{
    ...
    public class LiquidityGrabDetector : IDetector
    {
                                ...
                                double bodySize, double rangeSize, double atr, double tickSize)
        {
            ...
            if (_config.EnableDebug) _logger.Debug($"LiquidityGrabDetector: Nuevo {directionalBias} Grab en TF{tfMinutes} @ {grabPrice:F2} Score:{grabInfo.Score:F3}");
        }
        ...
        private double CalculateGrabScore(LiquidityGrabInfo grabInfo, int tfMinutes, int barIndex)
        {
            ...
            // Asegurar ventaja mínima por confirmación
            if (grabInfo.ConfirmedReversal)
            {
                score += 0.10; // aumentar ventaja de confirmación
            }
            ...
            foreach (var grabInfo in grabs)
            {
                ...
                if (!grabInfo.ConfirmedReversal && grabInfo.BarsSinceGrab <= _config.LG_MaxBarsForReversal)
                {
                    ...
                    if (grabInfo.DirectionalBias == "BuySide")
                    {
                        ...
                        {
                            // Confirmado - aplicar bonificación inmediata
                            grabInfo.ConfirmedReversal = true;
                            double oldScore = grabInfo.Score;
                            // Recalcular score AHORA con la bonificación de confirmación
                            double newScore = CalculateGrabScore(grabInfo, tfMinutes, barIndex);
                            // Garantizar monotonía: confirmado ≥ no confirmado + epsilon
                            grabInfo.Score = Math.Min(1.0, Math.Max(newScore, oldScore + 0.12));
                            _logger.Debug($"LiquidityGrabDetector: Grab {grabInfo.Id} confirmado - Score: {oldScore:F3} → {grabInfo.Score:F3}");
                            // No llamar a UpdateStructure para evitar que el engine sobreescriba el score del detector
                        }
                    }
                ...
                {
                    // Para grabs confirmados, mantener score establecido por el detector
                    // No llamar a UpdateStructure para evitar recálculo genérico
                }
                else
                {
