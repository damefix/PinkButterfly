    ...
    public class ScoringEngine
    {
        ...
        public double CalculateScore(StructureBase structure, int currentBarIndex, string marketBias, int confluenceCount = 1)
        {
            try
            {
                ...
                // ============================================================
                // Usar cierre del mismo TF para proximidad (coherente con tests)
                double currentPrice = _provider.GetClose(structure.TF, currentBarIndex);
                ...
                {
                    // Usar SIEMPRE el borde más cercano al precio actual (mejor correlación con entrada real)
                ...
                if (structure is FVGInfo fvgOld)
                {
                ...
                if (structure is FVGInfo fvgTfBonus)
                {
                ...
                if (structure is FVGInfo fvgProx)
                {
                    if (proxMaxTicks > 0 && distanceTicks >= proxMaxTicks)
                    {
